    ...
    stages {
        ...
        stage('Deploy container'){
            steps{
                ...
                && docker run -d --name ${env.NameContainer} -p 5000:5000
                ${env.RepoDockerHub}/${env.NameContainer}:${env.BUILD_NUMBER} ; else docker run -d --name
                pokedex-flask -p 5000:5000 ${env.RepoDockerHub}/${env.NameContainer}:${env.BUILD_NUMBER} ; fi"
            }
        }        
    }
}        
